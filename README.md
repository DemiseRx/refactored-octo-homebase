...
If you want a turnkey Pinokio setup, skip the manual steps above and follow the
packaged workflow described in [Section 5](#5-customisation--pinokio-integration).
...
- **Pinokio drop-in**: the repository now ships with a `pinokio_api/` folder.
  Drag the entire project into Pinokio's `api` directory and the app will appear
  with two buttons—**Install dependencies** and **Start service**—backed by the
  bundled shell scripts. The start action is marked for auto-start so you can
  enable one-click narration directly from Pinokio's UI. Advanced users can
  customise the dependency specifications via the environment variables
  documented in `pinokio_api/README.md`.
